...
func TestSerializeDynMapWithTwoColumnsReturnsTheBytesExpected(t *testing.T) {
	...
	var bytesExpected []byte
	bytesNCols := SerializeUint(uint32(2))
	bytesLenKey := SerializeUint(uint32(len("test")))
	bytesKey := []byte("test")
	bytesLenValue := SerializeUint(uint32(4))
	bytesValue := SerializeUint(uint32(32))
	bytesLenKey2 := SerializeUint(uint32(len("test_string")))
	bytesKey2 := []byte("test_string")
	bytesLenValue2 := SerializeUint(uint32(len(stringCol2)))
